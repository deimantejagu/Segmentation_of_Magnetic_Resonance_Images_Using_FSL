...
class WN_ConvTranspose3d(nn.ConvTranspose3d):
    ...
    def forward(self, input, output_size=None):
        ...
        if self.in_channels == self.out_channels:
            ...
            norm_factor = torch.sqrt((self.weight ** 2).sum(dim=(2, 3, 4), keepdim=True) + 1e-6)
            norm_weight = self.weight * (weight_scale[None, :, None, None, None] / norm_factor)
        output_padding = self._output_padding(input, output_size, self.stride, self.padding, self.kernel_size, len(self.kernel_size))
